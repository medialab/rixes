...
"""
NER pipeline (with evaluation) to detect locations without semantic anchoring around keywords.
"""
...
# check character matching (error rate tolerated : 3 characters)
def is_approx_match(gt, pred_str, max_distance=3):
    ...
    for pred in preds:
        if lev.distance(gt, pred) <= max_distance:
            return 1  # if at least one fuzzy match, match included
