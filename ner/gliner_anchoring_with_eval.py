...
MAX_TOKENS = 384 # to avoid truncation
TARGET_WORDS = ['rixe', 'bagarre'] # anchor words 
tqdm.pandas() 

# create chunks around target words rixe and bagarre
...
# predict the city closest to an anchor word
def find_closest_city(text):
    ...
    for chunk in chunks:
        try:
            entities = model.predict_entities(chunk, labels=['ville'])
            all_location_entities.extend([e for e in entities if e['label'] == 'ville'])
        except Exception as e:
            print(f"Prediction error (ville): {e}")
...
# predict region closest to an anchor word
...
# check if two strings match (error rate tolerated : 3 characters)
...
if __name__ == "__main__":
    ...
    print("Ville")
    print(f"precision : {precision_ville:.2f}")
    print(f"recall : {recall_ville:.2f}")

    print("Région")
    print(f"precision : {precision_region:.2f}")
    print(f"recall : {recall_region:.2f}")